...
export async function POST(request: NextRequest) {
  ...
  if (!rateLimitResult.success) {
    return addRateLimitHeaders(
      withCors(NextResponse.json({ error: '요청이 너무 많습니다. 잠시 후 다시 시도해주세요.' }, { status: 429 })),
      rateLimitResult
    )
  }
  ...
  try {
    if (!process.env.NOTION_TOKEN || !process.env.NOTION_CREATOR_DB_ID) {
      console.error('[Creator API] NOTION_TOKEN 또는 NOTION_CREATOR_DB_ID 미설정')
      return withCors(NextResponse.json({ error: '서버 설정 오류입니다.' }, { status: 500 }))
    }

    const body = await request.json()
    const { name, email, phone, instagram_url, youtube_url, tiktok_url, x_url, message, track_type, locale } = body
    ...
    if (!has(instagram_url)) {
      return withCors(NextResponse.json({ error: 'Instagram URL을 입력해주세요.' }, { status: 400 }))
    }
    ...
    const safeYoutube = has(youtube_url) ? clamp(trim(youtube_url), MAX_LEN.url) : null
    const safeTiktok = has(tiktok_url) ? clamp(trim(tiktok_url), MAX_LEN.url) : null
    const safeX = has(x_url) ? clamp(trim(x_url), MAX_LEN.url) : null
    const safeMessage = has(message) ? clamp(trim(message), MAX_LEN.message) : null
    ...
    if (safeMessage) properties['Message'] = { rich_text: [{ text: { content: safeMessage } }] }

    const response = await notion.pages.create({
      parent: { database_id: process.env.NOTION_CREATOR_DB_ID },
      properties,
    })
    ...
    return addRateLimitHeaders(
      withCors(NextResponse.json({ success: true, pageId: response.id }, { status: 200 })),
      rateLimitResult
    )
  } catch (error: any) {
    ...
    return withCors(
      NextResponse.json(
        { error: process.env.NODE_ENV === 'production' ? '서버 오류가 발생했습니다.' : error.message },
        { status: 500 }
      )
    )
  }

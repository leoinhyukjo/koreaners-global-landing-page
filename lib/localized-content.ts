import type { Portfolio, BlogPost, BlogFAQ } from "@/lib/supabase";
...
export function getPortfolioClientName(p: Portfolio, locale: Locale): string {
  if (locale === "ja" && p.client_name_jp?.trim())
    return p.client_name_jp.trim();
...
export function getBlogMetaDescription(
  p: BlogPost,
  locale: Locale,
): string | null {
  if (locale === "ja" && p.meta_description_jp != null)
    return p.meta_description_jp;
